...
/**
 * True for non-virtual columns whose value renders as plain text or a number — the kind that
 * overflows a single line. Used to decide whether a truncation tooltip is worth showing
 * (e.g. on gallery / kanban card titles). Virtual cells (Formula, etc.) are excluded.
 */
export const isTextOrNumberColumn = (column?: ColumnType | null): boolean => {
  if (!column || isVirtualCol(column)) return false

  return (
    isNumericCol(column) ||
    [UITypes.SingleLineText, UITypes.LongText, UITypes.Email, UITypes.PhoneNumber, UITypes.URL].includes(column.uidt as UITypes)
  )
}
